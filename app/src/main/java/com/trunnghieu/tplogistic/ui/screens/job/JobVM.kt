...
import com.trunnghieu.tplogistic.data.repository.local.menu.SubMenuType
...
class JobVM : BaseRepoViewModel<AccountRepo, JobUV>() {
    ...
    val deliveryWorkFlowRepo = DeliveryWorkFlowRepo()
    ...
    // Driver data
    val driverName = appPrefs.getString(AppPrefs.DRIVER.FULL_NAME)
    val driverPhoneNo = appPrefs.getString(AppPrefs.DRIVER.MOBILE_NO)
//    val driverName = appPrefs.getString(AppPrefs.DRIVER.FULL_NAME)
     ...
     */
    fun logOut() {
        appPrefs.storeValue(AppPrefs.LOGIN.IS_LOGIN, false)
...
//        startUpdateDriverLocation()
     ...
     */
    fun refreshJobData() {
        deliveryWorkFlowRepo.getJobById(LocalJob.get().getLatestJob()?.id ?: "", object : BaseRepoCallback<Job> {
